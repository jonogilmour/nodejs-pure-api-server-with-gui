...
class Server {
  ...
  constructor() {
    ...
    const assets = {
      'favicon.ico': handlers.staticFiles('favicon'),
      'public': handlers.staticFiles('public')
    }
  ...
  unifiedServer(req, res) {
    ...
    req.on('end', () => {
      ...
      // Send the request to the handler for the particular path
      // - (/public/* requests go to static assets
      // - other requests go to respective handler if it exists
      // - otherwise send to not_found
      const chosenHandler =
        trimmedPath.match(/^public/)
        ? this.router.public
      ...
      chosenHandler(data, (statusCode, payload, contentType = 'json') => {
        ...
        res.setHeader(
          'Content-Type',
          helpers.contentType(contentType)
        );

        // Set the payload string depending on the contentType requested
        const payloadString = helpers.payloadToString(payload, contentType);
